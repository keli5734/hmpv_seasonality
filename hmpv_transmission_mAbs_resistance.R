...
hmpv_transmission_model_mAbs_resistance <- function(time, y, parms) {
  with(as.list(c(y, parms)), {
    ...
    #      my_beta <-  parm$baseline.txn.rate
    # } else {
    #   my_beta <- parm$baseline.txn.rate
    # }
    # 
    ...
    dy[,'Mab'] <- prop_mab * period.birth.rate * sum(States) - 
      ...
      f_int * RSV_incidence * Mab + eta * Rab
    ...
    dy[,'Rab'] <- f_int * RSV_incidence * Mab - 
      eta * Rab -
      AGE * Rab -
      mu * Rab[1:(N_ages)] +   
      delta*c(0,Rab[1:(N_ages-1)])   
    
    
    dy[,'R1'] <- f_int * RSV_incidence * M - 
      eta * R1 -
      AGE * R1 -
      mu * R1[1:(N_ages)] +   
      delta*c(0,R1[1:(N_ages-1)])   
    
    dy[,'R2'] <- f_int * RSV_incidence * S0 - 
      eta * R2 -
      AGE * R2 -
      mu * R2[1:(N_ages)] +   
      delta*c(0,R2[1:(N_ages-1)])   
    
    dy[,'R3'] <- f_int * RSV_incidence * S1- 
      eta * R3 -
      AGE * R3 -
      mu * R3[1:(N_ages)] +   
      delta*c(0,R3[1:(N_ages-1)])   
    
    dy[,'R4'] <- f_int * RSV_incidence * S2 - 
      eta * R4 -
      AGE * R4 -
      mu * R4[1:(N_ages)] +   
      delta*c(0,R4[1:(N_ages-1)])     
    
    dy[,'R5'] <- f_int * RSV_incidence * S3 - 
      eta * R5 -
      AGE * R5 -
      mu * R5[1:(N_ages)] +   
      delta*c(0,R5[1:(N_ages-1)])   

...
source("hmpv_transmission_model_mAbs_RSV.R")
...
prop_mabs = seq(0,1,0.1)


RSV_prediction = matrix(NA, nrow = 209, ncol = length(prop_mabs))
RSV_prediction_under_1 = matrix(NA, nrow = 209, ncol = length(prop_mabs))
RSV_prediction_1_yr = matrix(NA, nrow = 209, ncol = length(prop_mabs))
age_distribution = matrix(NA, nrow = 5, ncol = length(prop_mabs))

RSV_prediction_list = list()
RSV_prediction_under_1_list = list()
RSV_prediction_1_yr_list = list()
...
parm =  readRDS(paste0("parm_rsv/parm_",region, ".rds"))
 


if(region %in% c(7, 8,9)){
...
yinit.matrix[,c("Ex","Mab","S1", "I2", "S2", "I3", "S3", "I4")]  <-  0 # setting initial conditions

yinit.matrix[,'M']  <-  c(Pop1[1:6], rep(0,N_ages-6))
#yinit.matrix[,'Mab']  <-  c(Pop1[1:6], rep(0,N_ages-6))
...
for(pp in 1:length(prop_mabs)){

  prop_mab = prop_mabs[pp]
...
parm_for_fit$data_rsv = c(rep(RSV_average, length.out = length(B_part2)),RSV_incidence, rep(RSV_average, length.out = 105)) 
...
results <-  
  ode(y = yinit.vector, 
      t = my_times,  
      func = hmpv_transmission_model_mAbs_RSV, 
...
reporting_fraction_age = c(rep(parm$reporting_fraction, 12),
                           rep(parm$reporting_fraction, 4),
                           rep(parm$reporting_fraction, 2),
                           rep(parm$reporting_fraction, 2),
                           rep(parm$reporting_fraction, 1)
)
...
RSV_prediction[,pp] = tail(H_true, n = 209)
RSV_prediction_under_1[,pp] = tail(H_true_under_1, n = 209)
RSV_prediction_1_yr[,pp] = tail(H_true_1_yr, n = 209)
age_distribution[,pp] = agedist

print(paste0("Region: ", region, ", values: ", prop_mabs[pp], " is done!"))
}
RSV_prediction_list[[region]] = RSV_prediction
RSV_prediction_under_1_list[[region]] = RSV_prediction_under_1
RSV_prediction_1_yr_list[[region]] = RSV_prediction_1_yr
...
# saveRDS(RSV_prediction_list, "RSV_prediction_list_all.rds")
# saveRDS(RSV_prediction_under_1_list, "RSV_prediction_under_1_list_all.rds")
# saveRDS(RSV_prediction_1_yr_list, "RSV_prediction_1_yr_list_all.rds"))
# saveRDS(age_distribution_list, "age_distribution_list_all.rds")
 
 
 

RSV_prediction_list = readRDS("predictions_rsv/RSV_prediction_1_yr_list_all.rds")

alpha_vec <- seq(0,1,0.1) 
 
result_df_RSV <- do.call(rbind, lapply(seq_along(RSV_prediction_list), function(region_id) {

  V <- colSums( (RSV_prediction_list[[region_id]][105:209,] ), na.rm = T)
...
result_df_RSV <- result_df_RSV %>%
  mutate(region = factor(region, levels = c(1:10))) %>%
  mutate(per = per * 100)   

result_df_RSV %>% group_by(alpha) %>% summarise(x = mean(per),
                                                xmin = min(per), 
                                                xmax = max(per))
...
Fig4A <-
  ggplot() +
  geom_line(data = result_df_RSV, aes(x = alpha * 100, y = per, group = region, color = region))+
  geom_point(data = result_df_RSV, aes(x = alpha * 100, y = per, group = region, color = region))+
  theme_bw()+
  scale_color_d3() +
  theme(legend.position = "right")  +
  labs(x = "Coverage of RSV interventions (%)", y = "RSV cases averted (%)", color = "Region") +
  theme(axis.text.x = element_text(color="black",
                                   size = 20, angle=0),
        axis.text.y = element_text(color="black",
                                   size= 20, angle=0),
...
Fig4A
 ...
 # Fig4A = Fig4A + theme(axis.text.x = element_text(color="black",
 #                                          size = 20, angle=0),
 #               axis.text.y = element_text(color="black",
 #                                          size= 20, angle=0),
 #               text = element_text(size = 20)) 
 # Fig4B = Fig4B + theme(axis.text.x = element_text(color="black",
 #                                                  size = 20, angle=0),
 #                       axis.text.y = element_text(color="black",
 #                                                  size= 20, angle=0),
 #                       text = element_text(size = 20)) 
...
plot_df_RSV <- do.call(rbind, lapply(seq_along(RSV_prediction_list), function(region_id) {
  mat <- RSV_prediction_list[[region_id]][105:209,]
...
plot_df_RSV$time = rep(date3_interpolate[(784+104):992])

# Plot
S1<-  ggplot(plot_df_RSV, aes(x = time, y = value, color = coverage)) +
  geom_line() +
  facet_wrap(~ region, scales = "free_y") +
  labs(
    title = "RSV dynamics under RSV interventions",
    x = "Date",
    y = "# of LRT RSV cases"
  ) +
  theme_minimal() +
